    ...
    if (addrd->targeta) {
        ...
        case BLE_LL_ADDR_SUBTYPE_RPA:
            ...
            if (rl && ble_ll_resolv_rpa(addrd->targeta, rl->rl_local_irk)) {
                rxinfo->flags |= BLE_MBUF_HDR_F_INITA_RESOLVED;
                break;
            }
...
ble_ll_scan_rx_isr_end(struct os_mbuf *rxpdu, uint8_t crcok)
{
    ...
    if (scansm->cur_aux_data) {
        ...
        scansm->cur_aux_data = NULL;
        if (pdu_type != BLE_ADV_PDU_TYPE_ADV_EXT_IND) {
            goto scan_rx_isr_end_done;
        }
    }
#endif

    if (!crcok) {
        goto scan_rx_isr_end_ignore;
    }
                             ...
                             struct ble_ll_scan_addr_data *addrd)
{
    ...
    if (send_hci_report) {
        ...
        if ((rc < 0) && BLE_MBUF_HDR_WAIT_AUX(hdr)) {
            ...
            rc = ble_ll_sched_rmv_elem(&aux_data->sch);
            BLE_LL_ASSERT(rc == 0);
