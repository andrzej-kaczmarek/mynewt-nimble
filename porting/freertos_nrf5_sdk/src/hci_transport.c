...
/*
 * XXX This code is based on original UART transport from Nimble so contains
 * some unused code - needs to be cleaned.
 */
...
static void tx_single_char(void);
...
static void uart_event_handler(nrf_drv_uart_event_t * p_event, void* p_context)
{
    ...
    if (p_event->type == NRF_DRV_UART_EVT_RX_DONE) {
        ...
        /* Trigger RX for next character */
        ret = nrf_drv_uart_rx(&uart_inst, &uart_rx_buf[uart_rx_index++], 1);
        assert(ret == NRF_SUCCESS);

        if (++uart_rx_tail == UART_RX_BUF_SIZE) {
            uart_rx_tail = 0;
        }

        assert(uart_rx_tail != uart_rx_head);

        xTaskNotifyFromISR(uart_task_h, 1, eSetBits,
...
ble_hci_uart_config(void)
{
    ...
    int err_code = nrf_drv_uart_init(&uart_inst, &config, uart_event_handler);
...
static void uart_task_fn (void * pvParameter)
{
    uint32_t nv;
    int t;
    ...
    while (1) {
        ...
        ble_hci_uart_rx_char(NULL, uart_rx_buf[uart_rx_head]);
...
ble_hci_transport_init(void)
{
    ...
    xTaskCreate(uart_task_fn, "uart", configMINIMAL_STACK_SIZE, NULL, 2,
                &uart_task_h);
