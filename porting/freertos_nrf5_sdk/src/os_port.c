...
os_eventq_dflt_get(void)
{
    ...
    if (!q) {
        q = xQueueCreate(32, sizeof(struct os_event *));
    }
...
os_eventq_get(struct os_eventq *evq)
{
    ...
    ret = xQueueReceive(evq, &ev, portMAX_DELAY);
    assert(ret == pdPASS);
...
os_eventq_remove(struct os_eventq *evq, struct os_event *ev)
{
    ...
    count = uxQueueMessagesWaiting(evq);
    for (i = 0; i < count; i++) {
        ret = xQueueReceive(evq, &tmp_ev, 0);
        assert(ret == pdPASS);
        ...
        ret = xQueueSendToBack(evq, &tmp_ev, 0);
        assert(ret == pdPASS);
    }
